...
func (g *SimplePoint) AppendBinary(dst []byte) []byte {
	dst = append(dst, ':', binSimplePoint)
	dst = appendBinaryPoint(dst, g.Point)
	return dst
}

func (g *SimplePoint) Binary() []byte {
...
func parseBinarySimplePointObject(src []byte, opts *ParseOptions) (*SimplePoint, int) {
	mark := len(src)
	if len(src) < 16 {
		return nil, 0
	}
	p := &SimplePoint{}
	p.Point = parseBinaryPoint(src)
	src = src[16:]
